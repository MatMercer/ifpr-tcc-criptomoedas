...
% TODO:
%O CAPÍTULO 2 SUGIRO TER A SEGUINTE ESTRUTURA:
%2.1 - CRIPTOMOEDAS (aqui você deve escrever sobre: definição,
%quantas moedas existem, quais são os tipos, todas são de tokens?,
%existe o smart contract – o que são eles....você deve aprofundar
%melhor no texto)
%O LEITOR PRECISA ENTENDER SOBRE AS CRIPTOMOEDAS E AS
%SUAS ESTRUTURAS E TIPOS
%2.1.1 – MOEDAS (aqui você iria descrever a ESTRUTURA do BITCOIN,
%EHTEREUM E MONERO)
...
%uma para as outras?, o são rigs?, o que são e como funcionam os
%pools?, explicar o que é e como funciona o processamento da CPU e
%GPU, ...aprofundar melhor)

%2.3 – Distribuição Linux (conceitos, tipos, derivações, características
%principais, inclua as distribuições e os diferenciais, crie um quadro
%para comparar as distribuições que explicar..)
...
Bitcoin. Existem aproximadamente mais de 800 \emph{alt coins} moedas
registradas no site Coin Market Cap \cite{Arsov, CoinMarketCap2018}.
Cada uma dessas moedas foram criadas para aplicações específicas,
diferentes do Bitcoin.  Suas implementações diferentes de criptomoedas
são (a) as de tokens e (b) as de contratos inteligentes.

O Bitcoin serve para transferir tokens, que são a representação de
algo transferível. Na implementação desta moeda, seus tokens possuem
valor monetário. Já o Ethereum é uma moeda alternativa que foca na
implementação de uma linguagem de programação Turing Completa para a
criação de scripts ou ``contratos inteligentes'' \cite{Narayanan2016}. 

O contrato inteligente é um programa que é executado no Blockchain
para executar regras de negócio específicas. Contratos inteligentes
permitem que qualquer aplicação possa ser implementada em uma mesma
moeda. Uma boa analogia de um contrato inteligente na prática é um
contrato para a prestação de serviços de uma empresa fictícia.

\begin{figure}[H]
    \caption{\label{fig:contratos-inteligentes}Exemplo de um contrato
    inteligente.}
    \begin{center}
        \includegraphics[width=.6\linewidth]{contratos-inteligentes.png}
    \end{center}
